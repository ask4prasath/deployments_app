class DeployHistoriesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @deploy_history.save
        ...
        format.json { render :show, status: :created, location: @deploy_history }
      else
  ...
  def update
    respond_to do |format|
      if @deploy_history.update(deploy_history_params)
        ...
        format.json { render :show, status: :ok, location: @deploy_history }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to deploy_histories_url, notice: 'Deploy history was successfully destroyed.' }
  ...
  private
    ...
    def deploy_history_params
      params.require(:deploy_history).permit(:release, :target_config_name, :mode, :comments)
    end
end
